...
impl FlussSys {
    pub(super) fn upsert_duckdb(
        ...
        let upsert = format!(
            "INSERT INTO {FLUSS_CHECKPOINTS_TABLE_NAME} (dataset_name, checkpoint_data, created_at, updated_at)
             VALUES (?, ?, now(), now())
    ...
    pub(super) fn get_duckdb(
        ...
        let query = format!(
            "SELECT checkpoint_data, epoch(updated_at) FROM {FLUSS_CHECKPOINTS_TABLE_NAME} WHERE dataset_name = ?"
        );
