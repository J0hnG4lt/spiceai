...
use super::{FLUSS_CHECKPOINTS_TABLE_NAME, FlussCheckpointMetadata, FlussSys, Error, Result};
...
impl FlussSys {
    pub(super) async fn upsert_sqlite(
        ...
        metadata: &FlussCheckpointMetadata,
    ) -> Result<()> {
            ...
            .call(
                move |conn: &mut rusqlite::Connection| -> Result<(), rusqlite::Error> {
                    let create_table = format!(
                        "CREATE TABLE IF NOT EXISTS {FLUSS_CHECKPOINTS_TABLE_NAME} (
                    ...
                    let upsert = format!(
                        "INSERT INTO {FLUSS_CHECKPOINTS_TABLE_NAME}
    ...
    pub(super) async fn get_sqlite(
        &self,
        pool: &SqliteConnectionPool,
    ) -> Option<FlussCheckpointMetadata> {
        let dataset_name = self.dataset_name.clone();
        ...
        conn.conn
            .call(move |conn: &mut rusqlite::Connection| -> Result<FlussCheckpointMetadata, rusqlite::Error> {
                let query = format!(
                    "SELECT checkpoint_data, strftime('%s', updated_at) FROM {FLUSS_CHECKPOINTS_TABLE_NAME} WHERE dataset_name = ?"
                );
